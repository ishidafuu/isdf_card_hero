...
export function attackWithCommand(state: GameState, action: CommandAction): GameState {
  ...
  const cost = getCommandStoneCost(attacker, command);
  ...
  if (applyUtilityCommandEffect(next, action.attackerSlotKey, command, action.target, power)) {
    return next;
  }
...
export function playMagic(state: GameState, action: MagicAction): GameState {
  ...
  applyMagicEffect(next, def.id, action.target);
...
export function useMasterAction(
  ...
  if (actionId === "master_attack") {
    ...
    damageMonster(next, target.slotKey, power, "マスターアタック");
    return next;
  }
...
export function getCommandTargets(
  ...
  if (getCommandStoneCost(monster, command) > actor.stones) {
    return [];
  }
...
function getMagicTargetsByCardId(state: GameState, cardId: string): Target[] {
  ...
  if (cardId === "card_025" || cardId === "card_030" || cardId === "card_055" || cardId === "card_062" || cardId === "card_088" || cardId === "card_089" || cardId === "card_091") {
    return activeTargets;
  }
  if (cardId === "card_026" || cardId === "card_092") {
    return [...activeTargets, ...enemyMaster];
  }
  if (cardId === "card_027" || cardId === "card_028" || cardId === "card_058" || cardId === "card_059" || cardId === "card_060" || cardId === "card_086" || cardId === "card_090" || cardId === "card_094" || cardId === "card_095" || cardId === "card_097" || cardId === "card_098" || cardId === "card_119" || cardId === "card_125" || cardId === "card_129" || cardId === "card_130" || cardId === "card_148") {
    return activeTargets;
  }
  if (cardId === "card_029") {
...
function applyMagicEffect(state: GameState, cardId: string, target: Target): void {
  ...
  if (cardId === "card_025" || cardId === "card_030" || cardId === "card_089") {
    ...
    if (cardId === "card_030" && target.kind === "monster") {
      const second = findFirstOtherActiveSlot(state, target.slotKey);
      if (second) {
  ...
  if (cardId === "card_055" || cardId === "card_088" || cardId === "card_091") {
    applyShieldMagic(state, target, 2);
    return;
  }
  ...
  if (cardId === "card_028" && target.kind === "monster") {
    const monster = requireTargetMonster(state, target.slotKey);
    if (monster.level < getMonsterDef(monster.cardId).maxLevel && !monster.levelFixed && state.players[monster.owner].stones > 0) {
  ...
  if (cardId === "card_031" && target.kind === "monster") {
    const partnerSlotKey = findSwapPartnerSlot(state, target.slotKey);
    if (!partnerSlotKey) {
  ...
  if (cardId === "card_061" && target.kind === "monster") {
    const allySlotKey = firstActiveSlotOf(state, state.currentPlayer);
    if (allySlotKey) {
  ...
  if (cardId === "card_065" && target.kind === "monster") {
    shiftChangeWithFirstHandMonster(state, target.slotKey);
    return;
  }
  ...
  if (cardId === "card_094" && target.kind === "monster") {
    const monster = requireTargetMonster(state, target.slotKey);
    monster.powerModifier = (monster.powerModifier ?? 0) + 1;
    appendLog(state, `${monsterName(monster)}のパワーを1上げた`);
    return;
  }
  ...
  if (cardId === "card_097" && target.kind === "monster") {
    const monster = requireTargetMonster(state, target.slotKey);
    monster.actionCount = monster.actionLimit;
    appendLog(state, `${monsterName(monster)}を挑発した`);
    return;
  }
  ...
  if (cardId === "card_098" && target.kind === "monster") {
    const monster = requireTargetMonster(state, target.slotKey);
    monster.shadowCursed = true;
    appendLog(state, `${monsterName(monster)}にデスチェーンを付けた`);
    return;
  }
  ...
  if (cardId === "card_116") {
    refreshHand(state, state.currentPlayer);
    return;
  }
  ...
  if (cardId === "card_121") {
    state.players[state.currentPlayer].stones += 2;
    appendLog(state, `${playerLabel(state.currentPlayer)}はストーンを2個得た`);
    return;
  }
  ...
  if (cardId === "card_123") {
    forceDraw(state, state.currentPlayer, "カードサーチ");
    return;
  }

  if (cardId === "card_124") {
    state.players[state.currentPlayer].masterPowerBonus = (state.players[state.currentPlayer].masterPowerBonus ?? 0) + 1;
    appendLog(state, "マスター特技を一時的に強化した");
    return;
  }
  ...
  if (cardId === "card_148" && target.kind === "monster") {
    mirrorFirstEnemyMonster(state, target.slotKey);
    return;
  }
...
function applyShieldMagic(state: GameState, target: Target, strength: 1 | 2): void {
  if (target.kind !== "monster") {
...
function damageTargetByMagic(state: GameState, target: Target, power: number, source: string): void {
  if (target.kind === "master") {
    damageMasterByPower(state, target.playerId, power, source);
    return;
  }
  damageMonster(state, target.slotKey, power, source);
}
...
function shiftChangeWithFirstHandMonster(state: GameState, slotKey: SlotKey): void {
  const slot = state.slots[slotKey];
  ...
  const handIndex = player.hand.findIndex((card) => getCardDef(card.cardId).type === "monster");
  if (handIndex < 0) {
...
function mirrorFirstEnemyMonster(state: GameState, slotKey: SlotKey): void {
  const target = requireTargetMonster(state, slotKey);
  const sourceSlotKey = PLAYER_SLOT_ORDER[opponentOf(target.owner)].find((candidate) => state.slots[candidate].monster?.status === "active");
...
export function getMonsterCommands(monster: MonsterState): CommandDef[] {
  const def = getMonsterDef(monster.cardId);
  const commands = def.levels
    .filter((item) => item.level === monster.level)
...
function clearExpiredShields(state: GameState, playerId: PlayerId): void {
  for (const slotKey of PLAYER_SLOT_ORDER[playerId]) {
    const monster = state.slots[slotKey].monster;
    if (monster?.shielded || monster?.immune || monster?.oneShotShield) {
...
function applyUtilityCommandEffect(
  ...
  if (command.name === "ソウルスイッチ") {
    switchWithFirstHandMonster(state, attackerSlotKey);
    return true;
  }
...
function applyPostDamageCommandEffect(
  ...
  if (command.range === "piercing") {
    damagePiercedTarget(state, target.slotKey, power, command.name);
  }
}
...
function switchWithFirstHandMonster(state: GameState, slotKey: SlotKey): void {
  const slot = state.slots[slotKey];
  const current = slot.monster;
  if (!current) {
...
function damagePiercedTarget(state: GameState, targetSlotKey: SlotKey, power: number, source: string): void {
  ...
  damageMonster(state, behindSlotKey, power, `${source}貫通`);
}
