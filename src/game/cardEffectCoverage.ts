...
export const SIMPLIFIED_OR_PENDING_CARD_EFFECTS: readonly SimplifiedOrPendingCardEffect[] = [
  ...
  },
  {
    ...
    currentBehavior: "手動UIでは候補ボタンまたは盤面クリックでsecondaryTargetを選べる。未指定のルールAPI/CPU/オート検証では合法候補から決定的に補完する。",
    reason: "CPU/オート検証の経路で進行不能を避けるため。",
    followUp: "CPUが盤面価値を評価してsecondaryTargetを選ぶ。",
  ...
  },
  {
    ...
    currentBehavior: "手動UIでは選択パネルから対象手札、捨てる手札、サーチカテゴリを選べる。未指定のルールAPI/CPU/オート検証では決定的に補完する。",
    reason: "CPU/オート検証の経路で進行不能を避けるため。",
    followUp: "CPUがデッキ残量/盤面価値を見て手札や山札カテゴリを選ぶ。",
