...
export const SIMPLIFIED_OR_PENDING_CARD_EFFECTS: readonly SimplifiedOrPendingCardEffect[] = [
  ...
  },
  {
    ...
    currentBehavior: "手動UIでは候補ボタンまたは盤面クリックでsecondaryTargetを選べる。CPUは候補を展開して盤面価値で評価する。未指定のルールAPIでは合法候補から決定的に補完する。",
    reason: "ルールAPIを直接呼ぶテストやオート補完経路で進行不能を避けるため。",
    followUp: "CPUの長期価値や相手ターンの反撃まで含めたsecondaryTarget評価を追加する。",
  ...
  },
  {
    ...
    currentBehavior: "手動UIでは選択パネルから対象手札、捨てる手札、サーチカテゴリを選べる。CPUは候補を展開して手札価値/配置価値で評価する。未指定のルールAPIでは決定的に補完する。",
    reason: "ルールAPIを直接呼ぶテストやオート補完経路で進行不能を避けるため。",
    followUp: "CPUのデッキ残量、将来ターン、手札上限まで含めた手札/山札評価を追加する。",
  ...
  },
  {
    ...
    currentBehavior: "撃破、回復、強化、シールド、ウェイクアップ、移動後攻撃、複数対象、手札/山札選択を浅い評価で判断する。",
    reason: "公式AI再現ではなく、ルール破綻を起こさず検証相手として動くことを優先しているため。",
    followUp: "warning seed 427、468、479をもとに、移動多発と強候補見送りを改善する。",
  },
  {
