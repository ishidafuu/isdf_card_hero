...
function isNonGameplayNote(note) {
  ...
  return (
    ...
    /lv\d+からレベルアップで登場$/.test(normalized) ||
    note.includes("限定版") ||
    note.includes("非売品") ||
    note.includes("超レア") ||
    note.includes("入手困難") ||
...
function parseMonsterLevels(table, cardName) {
  ...
  return levels.map((level) => ({
    ...level,
    commands: level.commands.map((command, index) => {
      const range = command.range === "special"
        ? commandRange(command.rangeText ?? "", cardName, command.name, command.power > 0)
        : command.range;
      const duplicateIndex = level.commands
        .slice(0, index)
        .filter((previous) => previous.id === command.id).length;
      return {
        ...command,
        id: duplicateIndex === 0 ? command.id : `${command.id}_${duplicateIndex + 1}`,
        range,
        recoilDamage:
...
async function main() {
  ...
  for (const table of document.querySelectorAll("table.card_basic")) {
    ...
    const seedNames = typeLabel === "スーパー" ? superSeedNames(table) : [];
    const evolvesFrom = seedNames
      .map((seedName) => CARD_ID_OVERRIDES.get(seedName) ?? cardsByName.get(seedName)?.id)
      .filter(Boolean);
    const seedRole = evolvesFrom
      .map((seedId) => cards.find((card) => card.id === seedId))
      .find((card) => card?.type === "monster")?.role;
    const base = {
      id: cardIdFor(no, name),
      name,
      pool: typeLabel === "スーパー" ? "special" : undefined,
      evolvesFrom: evolvesFrom.length > 0 ? evolvesFrom : undefined,
      sourceNo: Number(no),
      sourceUrl: detailUrl,
    ...
    if (typeLabel === "マジック") {
      ...
      cards.push(parseMonster(table, base, typeLabel === "後衛" ? "back" : seedRole ?? "front"));
    }
    cardsByName.set(name, cards.at(-1));
  }
  ...
  const generated = `import type { CardDef } from \"./types\";\n\n` +
    ...
    `// Official super cards are imported as pool: \"special\" and are only used through super level-up.\n` +
    `export const CARD_DEFS = ${JSON.stringify(cardDefs, null, 2)} satisfies Record<string, CardDef>;\n`;

  await writeFile(DATA_PATH, generated);
  console.log(`Imported ${cards.length} cards and icons.`);
}

function superSeedNames(table) {
  const seedText = text(table.querySelector(".mn_name_seed"));
  return seedText
    .replace(/\s*Lv\d+からレベルアップで登場.*$/u, "")
    .split(/\s+/)
    .map((name) => name.trim())
    .filter(Boolean);
}
