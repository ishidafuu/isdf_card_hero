...
| `aiUnitTraits` | コマンド射程、マスター圧、柔軟射程、ユニット配置適性などの意味分類 |
...
- コマンド/ユニットの評価補助は `src/game/aiUnitTraits.ts` で分類する。
- `scoreMagicDecision` はカードIDではなくtraitの `valueModel` / `effectKind` で評価関数を選ぶ。
- ダメージ魔法の脅威推定は固定威力ではなく、`playMagic` の仮実行後にHP差分を見る。
- 評価係数は `src/game/aiWeights.ts` に分離し、`stable` / `strong` / `pressure` / `defensive` のプロファイル別に調整できる。
- `strong` は初期値として、マスターHP差とマスター打点をやや重く、ストーン消費とモンスター撃破価値をやや軽く見る。
- `pressure` / `defensive` は既定AIではなく、相性別の比較・調整候補を探すための検証プロファイルとして扱う。
...
5. 調整前に `npm run simulate:card-adjustment -- --card <id> ...` で影響見積もりとレビュー対象を確認する。
6. `npm run safety:card-adjustment` でtraits、短い実戦スコア、AI重み比較、閾値チェックをまとめて確認する。
7. `npm run improve:ai-loop` で次に回すべきtrace、追加母数、重み比較を自動生成する。
8. 必要に応じて `npm run diff:ai` と `npm run benchmark:deck-suite` で判断変化を確認する。
